...
class RootApi(Resource):
    '''
    curl -X GET http://localhost:8000/v1/hello --header "Content-Type: application/json"
    curl -X POST http://localhost:8000/v1/hello --header "Content-Type: application/json" --data '{"RequestType":"Hello"}'
    '''
    def get(self):
